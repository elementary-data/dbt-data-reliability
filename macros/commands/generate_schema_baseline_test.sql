{% macro generate_schema_baseline_test(
    name=none,
    ...
    {% if name %}
        {{
            generate_schema_baseline_test_for_node(
                name,
    ...
    {% else %}
        {{
            generate_schema_baseline_test_for_all_nodes(
...
{% macro generate_schema_baseline_test_for_all_nodes(
    include_sources=True,
    include_models=False,
    fail_on_added=False,
    enforce_types=False,
    ...
    {% for node in nodes %}
        {% if node.package_name != "elementary" and (
            (include_sources and node.resource_type == "source")
            or (include_models and node.resource_type == "model")
        ) %}
            {% do print(
                "Generating schema changes from baseline test for {} '{}':".format(
...
{% macro generate_schema_baseline_test_for_node(
    node,
    fail_on_added=False,
    enforce_types=False,
    convert_to_lower=False,
    ...
    {% if node is string %}
        ...
        {% if not node %}
            {% do print(
                "Could not find any model or source by the name '{}'!".format(
                    node_name
                )
            ) %}
            {% do return(none) %}
...
{% macro generate_schema_baseline_test_for_source(
    node, columns, test_params, convert_to_lower
) %}
    sources: - name:{{ node.source_name }}
    tables: - name:{{ node.name }}
    columns:
    {%- for column in columns %}
        - name:{{ column.name }}
        {%- if convert_to_lower %} data_type:{{ column.dtype | lower }}
        {% else %} data_type:{{ column.dtype }}
        {% endif -%}
    {% endfor %}
    tests: - elementary.schema_changes_from_baseline
    {%- if test_params %}
        :
...
{% macro generate_schema_baseline_test_for_model(
    node, columns, test_params, convert_to_lower
) %}
    models: - name:{{ node.name }}
    columns:
    {%- for column in columns %}
        - name:{{ column.name }}
        {%- if convert_to_lower %}
            data_type:{{ elementary.get_column_data_type(column) | lower }}
        {% else %} data_type:{{ elementary.get_column_data_type(column) }}
        {% endif -%}
    {% endfor %}
    tests: - elementary.schema_changes_from_baseline
    {%- if test_params %}
