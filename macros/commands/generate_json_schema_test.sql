{% macro generate_json_schema_test(node_name, column_name) %}
    {% if target.type not in ["snowflake", "bigquery"] %}
        {% do exceptions.raise_compiler_error(
            "JSON schema test generation is not supported for target: {}".format(
                target.type
            )
        ) %}
    {% endif %}
    ...
    {% if node.resource_type not in ["source", "model"] %}
        {% do exceptions.raise_compiler_error(
            "Only sources and models are supported for this macro, supplied node type: '{}'".format(
                node.resource_type
            )
        ) %}
    {% endif %}
    ...
    {% if not elementary.column_exists_in_relation(node_relation, column_name) %}
        {% do exceptions.raise_compiler_error(
            "Column '{}' does not exist in {} '{}'!".format(
                column_name, node.resource_type, node_name
            )
        ) %}
    {% endif %}

    {% set (
        elementary_database_name,
        elementary_schema_name,
    ) = elementary.get_package_database_and_schema() %}

    {% do node.config.update({"packages": ["genson"]}) %}
    {% do node.update(
        {
            "database": elementary_database_name,
            "schema": elementary_schema_name,
        }
    ) %}
    ...
    {% set output_table = api.Relation.create(
        database=elementary_database_name,
        schema=elementary_schema_name,
        identifier="json_schema_tmp__" ~ node.alias,
    ).quote(false, false, false) %}

    {% set gen_json_schema_func = elementary.generate_json_schema_py_func(
        column_name
    ) %}
    {% set node_relation = node_relation.quote(false, false, false) %}
    {% set compiled_py_code = adapter.dispatch("compile_py_code", "elementary")(
        node_relation,
        gen_json_schema_func,
        output_table,
        code_type="function",
    ) %}

    {% do elementary.run_python(node, compiled_py_code) %}
    {% set json_schema = elementary.result_value(
        "select result from {}".format(output_table)
    ) %}
    {% if json_schema == "genson_not_installed" %}
        {% do exceptions.raise_compiler_error(
            "The 'genson' python library is missing from your warehouse.\n\n"
            ...
            "regarding how to install python packages for a {} warehouse.".format(
                target.type
            )
        ) %}
    {% endif %}
    ...
    {% if not json_schema %}
        {% do exceptions.raise_compiler_error(
            "Not a valid JSON column: {}".format(column_name)
        ) %}
    {% endif %}
...
columns:
  - name: {{ column_name }}
    ...
    {% do print(
        "Please add the following test to your {} configuration for the column {}:".format(
            node.resource_type, column_name
        )
    ) %}
    {% do print(testyaml) %}
{% endmacro %}
