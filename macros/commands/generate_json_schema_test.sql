{% macro generate_json_schema_test(node_name, column_name) %}
    ...
    {% set node = elementary.dict_update(node, {'config': elementary.dict_update(node.config, {"packages": ["genson"]})}) %}
    {% set node = elementary.dict_update(node, {'database': elementary_database_name, 'schema': elementary_schema_name}) %}
    {% if node.resource_type == 'source' %}
      {# Source nodes don't have alias, and submit_python_job expects it #}
      {% set node = elementary.dict_update(node, {'alias': "jsonschemagen_{}_{}".format(node.source_name, node.name)}) %}
    {% endif %}
