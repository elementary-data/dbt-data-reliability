{% macro get_columns_and_types(table_name, schema_name=none, database_name=none) %}
    ...
    {%- endif %}
    {%- if not schema_name %} {%- set schema_name = target.schema %} {%- endif %}

    {%- set columns = [] %}

    {%- set relation = adapter.get_relation(
        database=database_name, schema=schema_name, identifier=table_name
    ) -%}
    ...
    {% for column in columns_from_relation %}
        {%- set column_item = {
            ...
            "data_type": elementary.normalize_data_type(column["dtype"]),
        } %}
