{% macro insert_rows(table_relation, rows, should_commit=false, chunk_size=5000) %}
    {% if not rows %} {{ return(none) }} {% endif %}

    {% if not table_relation %}
        {{
            elementary.edr_log(
                'Unable to insert rows to non-existent table. Please run "dbt run -s elementary --target '
                ~ target.name
                ~ '"'
            )
        }}
    ...
    {% elif insert_rows_method == "chunk" %}
        ...
        {% for rows_chunk in rows_chunks %}
            {% set insert_rows_query = elementary.get_chunk_insert_query(
                table_relation, columns, rows_chunk
            ) %}
            {% do elementary.run_query(insert_rows_query) %}
        {% endfor %}
    {% else %}
        {% do exceptions.raise_compiler_error(
            "Specified invalid value for 'insert_rows_method' var."
        ) %}
    {% endif %}
...
{% macro get_insert_rows_queries(
    ...
    columns,
    rows,
    query_max_size=elementary.get_config_var("query_max_size")
) -%}
    {% set insert_queries = [] %}
    {% set insert_query %}
    ...
    {% for row in rows %}
        ...
        {% if query_with_row | length > query_max_size %}
            ...
            {% if new_insert_query | length > query_max_size %}
                {% do elementary.file_log(
                    "Oversized row for insert_rows: {}".format(query_with_row)
                ) %}
                {% do exceptions.raise_compiler_error(
        ...
        {% else %} {% set current_query.data = query_with_row %}
        {% endif %}
        {% if loop.last %}
            {% do insert_queries.append(current_query.data) %}
        {% endif %}
