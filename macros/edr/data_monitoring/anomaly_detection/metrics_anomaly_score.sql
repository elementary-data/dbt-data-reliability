{% macro get_metrics_anomaly_score_query(full_table_name, monitors, column_name = none) %}
    {%- set timeframe_end = "'"~ run_started_at.strftime("%Y-%m-%d 00:00:00")~"'" %}
    {% set metrics_anomaly_score_query %}
        with data_monitoring_metrics as (

            select * from {{ ref('data_monitoring_metrics') }}
            where upper(full_table_name) = upper('{{ full_table_name }}') and metric_name in {{ elementary.strings_list_to_tuple(monitors) }}
            {% if column_name %}
        ...
        select * from final where is_anomaly = true
    {% endset %}
    {{ return(metrics_anomaly_score_query) }}
{% endmacro %}