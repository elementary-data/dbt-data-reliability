{% macro table_monitoring_query(
    ...
    with
        ...
        metrics as (
            {{
                elementary.get_unified_metrics_query(
                    metrics=table_monitors, metric_properties=metric_properties
                )
            }}
        ),
        ...
        {% if timestamp_column %}
            metrics_final as (
                ...
                select
                    ...
                    {{
                        elementary.timediff(
                            "hour", "edr_bucket_start", "edr_bucket_end"
                        )
...
{% macro get_unified_metrics_query(metrics, metric_properties) %}
    ...
    {%- for metric_name in metrics %}
        {%- set metric_query = elementary.get_metric_query(
            metric_name, metric_properties
        ) %}
        {%- if metric_query %}
    ...
    {% if not included_monitors %}
        ...
        {% else %}
            {% do return(
                elementary.empty_table(
                    [("metric_name", "string"), ("metric_value", "int")]
                )
            ) %}
        {% endif %}
    {% endif %}
...
{% macro get_metric_query(metric_name, metric_properties) %}
    ...
    {%- if not metric_macro %} {%- do return(none) %} {%- endif %}

    {%- set metric_query = metric_macro(metric_properties) %}
    {%- if not metric_query %} {%- do return(none) %} {%- endif %}
    ...
    {% if metric_properties.timestamp_column %}
        ...
        with
            unique_timestamps as (
                select distinct
                    {{ elementary.edr_cast_as_timestamp(freshness_column) }}
                    as timestamp_val
                from monitored_table
                order by 1
            ),

            -- compute freshness for every update as the time difference from the
            -- previous update
            ...
            bucketed_consecutive_updates_freshness as (
                ...
                from buckets
                cross join consecutive_updates_freshness
                where
            ...
            -- the time that passed since the last update in the bucket and the end of
            -- the bucket.
            ...
            -- get all the freshness values, ranked by size (we use partition by and
            -- not group by, because we also want to have
...
{% macro event_freshness_metric_query(metric_properties) %}
    ...
    {% if update_timestamp_column %}
        select
            ...
            {{
                elementary.edr_cast_as_string(
                    "max({})".format(event_timestamp_column)
                )
            }} as source_value,
            {{
                "coalesce(max({}), {})".format(
                    elementary.timediff(
                        "second",
                        elementary.edr_cast_as_timestamp(event_timestamp_column),
                        elementary.edr_cast_as_timestamp(update_timestamp_column),
                    ),
                    elementary.timediff(
        ...
        from buckets
        left join
            time_filtered_monitored_table on (edr_bucket_start = start_bucket_in_data)
        group by 1, 2
    {% else %}
        select
            {{ elementary.const_as_string("event_freshness") }} as metric_name,
