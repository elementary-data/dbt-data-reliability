{% macro table_monitoring_query(monitored_table_relation, timestamp_column, is_timestamp, min_bucket_start, table_monitors, period, freshness_column=none) %}
    ...
    {% if is_timestamp %}
        ...
        period_row_count as (
            ...
            from period_buckets left join filtered_monitored_table on (edr_period_bucket = start_bucket_in_data)
            group by 1,2
        ),
        ...
        {%- if 'freshness' in table_monitors %}
            ...
            {%- endif %}
            select
                edr_period_bucket as edr_bucket,
                {{ elementary.const_as_string('freshness') }} as metric_name,
                {{ elementary.cast_as_string('max('~freshness_column~')') }} as source_value,
                {{ elementary.timediff('second', elementary.cast_as_timestamp('max('~freshness_column~')'), elementary.timeadd(period,'1','edr_period_bucket')) }} as metric_value
            from period_buckets, {{ monitored_table_relation }}
            where {{ elementary.cast_as_timestamp(timestamp_column) }} <= {{ elementary.timeadd(period,'1','edr_period_bucket') }}
            group by 1,2
        {%- else %}
            {{ elementary.empty_table([('edr_bucket','timestamp'),('metric_name','string'),('source_value','string'),('metric_value','int')]) }}
        {%- endif %}
