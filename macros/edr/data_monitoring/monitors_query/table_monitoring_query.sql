...
{% if metric_properties.timestamp_column %}
    ...
    bucket_end_freshness as (
        select
            ...
            {{ elementary.timediff('second', elementary.edr_cast_as_timestamp('max(timestamp_val)'), "least(edr_bucket_end, {})".format(elementary.edr_current_timestamp_column())) }} as freshness
