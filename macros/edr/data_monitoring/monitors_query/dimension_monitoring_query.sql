...
{% macro default__dimension_monitoring_query(monitored_table_relation, dimensions, min_bucket_start, metric_properties) %}
...
{% macro sqlserver__dimension_monitoring_query(monitored_table_relation, dimensions, min_bucket_start, metric_properties) %}
    {% set metric_name = 'dimension' %}
    {% set full_table_name_str = elementary.quote(elementary.relation_to_full_name(monitored_table_relation)) %}
    {% set dimensions_string = elementary.join_list(dimensions, '; ') %}
    {% set concat_dimensions_sql_expression = elementary.list_concat_with_separator(dimensions, '; ') %}
        ...
        from (
            ...
            from (
                ...
                from (
                    ...
                    from (
                        ...
                        from (
                            ...
                            from (
                                ...
                                having sum(metric_value) > 0
                            ) dimension_values_without_outdated
                            union all
                            select distinct
                                dimension_value,
                                1 as joiner
                            from (
                                ...
                                from {{ monitored_table_relation }}
                                where
                                    {{ elementary.cast_as_timestamp(timestamp_column) }} >= (select min(edr_bucket_start) from (
                                        select
                                            edr_bucket_start,
                                            edr_bucket_end,
                                            1 as joiner
                                    ...
                                    and {{ elementary.cast_as_timestamp(timestamp_column) }} < (select max(edr_bucket_end) from (
                                        select
                                            edr_bucket_start,
                                            edr_bucket_end,
                                            1 as joiner
                    ...
                    {# {{ dbt_utils.group_by(2) }} #}
                    group by start_bucket_in_data, {{ concat_dimensions_sql_expression }}
                ) filtered_row_count_values on (edr_bucket_start = start_bucket_in_data and dimensions_buckets.dimension_value = filtered_row_count_values.dimension_value)
