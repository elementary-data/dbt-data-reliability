        ...
        {%- if column_config %}
            {%- for monitored_column in column_config -%}
                ...
                select
                    ...
                    {%- if 'max' in column_monitors -%} max(cast({{ column_name }} as {{ dbt_utils.type_int() }})) {%- else -%} null {% endif %} as max,
                    {%- if 'min' in column_monitors -%} min(cast({{ column_name }} as {{ dbt_utils.type_int() }})) {%- else -%} null {% endif %} as min,
                    {%- if 'average' in column_monitors -%} avg(cast({{ column_name }} as {{ dbt_utils.type_int() }})) {%- else -%} null {% endif %} as average,
                    ...
                    {%- if 'standard_deviation' in column_monitors -%} stddev(cast({{ column_name }} as {{ dbt_utils.type_int() }})) {%- else -%} null {% endif %} as standard_deviation,
                    {%- if 'variance' in column_monitors -%} variance(cast({{ column_name }} as {{ dbt_utils.type_int() }})) {%- else -%} null {% endif %} as variance,
                    {%- if 'max_length' in column_monitors -%} max(length({{ column_name }})) {%- else -%} null {% endif %} as max_length,
                    {%- if 'min_length' in column_monitors -%} min(length({{ column_name }})) {%- else -%} null {% endif %} as min_length,
                    {%- if 'average_length' in column_monitors -%} avg(length({{ column_name }})) {%- else -%} null {% endif %} as average_length,
