{% macro column_monitoring_query(monitored_table, monitored_table_relation, min_bucket_start, max_bucket_end, days_back, column_obj, column_metrics, metric_properties, dimensions) %}
    {{ return(adapter.dispatch('column_monitoring_query', 'elementary')(monitored_table, monitored_table_relation, min_bucket_start, max_bucket_end, days_back, column_obj, column_metrics, metric_properties, dimensions)) }}
{% endmacro %}

{% macro default__column_monitoring_query(monitored_table, monitored_table_relation, min_bucket_start, max_bucket_end, days_back, column_obj, column_metrics, metric_properties, dimensions) %}
...
{% macro fabric__column_monitoring_query(monitored_table, monitored_table_relation, min_bucket_start, max_bucket_end, days_back, column_obj, column_metrics, metric_properties, dimensions) %}
    {%- set full_table_name_str = elementary.edr_quote(elementary.relation_to_full_name(monitored_table_relation)) %}
    {%- set timestamp_column = metric_properties.timestamp_column %}
    {% set prefixed_dimensions = [] %}
    {% for dimension_column in dimensions %}
    ...
    -- Main select without WITH clauses for Fabric compatibility
    select
        {{ elementary.generate_surrogate_key([
            'full_table_name',
            'column_name',
        ...
        metrics_final.full_table_name,
        metrics_final.column_name,
        metrics_final.metric_name,
        metrics_final.metric_type,
        metrics_final.metric_value,
        metrics_final.source_value,
        metrics_final.bucket_start,
        metrics_final.bucket_end,
        metrics_final.bucket_duration_hours,
        {{ elementary.edr_current_timestamp_in_utc() }} as updated_at,
        metrics_final.dimension,
        metrics_final.dimension_value,
        metrics_final.metric_properties
    from (
        -- Metrics final
        select
            {{ elementary.edr_cast_as_string(full_table_name_str) }} as full_table_name,
            column_metrics_unpivot.edr_column_name as column_name,
            column_metrics_unpivot.metric_name,
            column_metrics_unpivot.metric_type,
            {{ elementary.edr_cast_as_float('column_metrics_unpivot.metric_value') }} as metric_value,
            {{ elementary.null_string() }} as source_value,
            column_metrics_unpivot.bucket_start,
            column_metrics_unpivot.bucket_end,
            column_metrics_unpivot.bucket_duration_hours,
            column_metrics_unpivot.dimension,
            column_metrics_unpivot.dimension_value,
            {{elementary.dict_to_quoted_json(metric_properties) }} as metric_properties
        from (
            -- Column metrics unpivot
            {%- if column_metrics %}
                {% for metric_name, metric_type in metric_name_to_type.items() %}
                    select
                        {{ elementary.const_as_string(column_obj.name) }} as edr_column_name,
                        column_metrics.bucket_start,
                        column_metrics.bucket_end,
                        {% if timestamp_column %}
                            {{ elementary.timediff("hour", "column_metrics.bucket_start", "column_metrics.bucket_end") }} as bucket_duration_hours,
                        {% else %}
                            {{ elementary.null_int() }} as bucket_duration_hours,
                        {% endif %}
                        {% if dimensions | length > 0 %}
                        ...
                        {% else %}
                          ...
                          {{ elementary.null_string() }} as dimension_value,
                        {% endif %}
                        {{ elementary.edr_cast_as_float('column_metrics.' ~ metric_type) }} as metric_value,
                        {{ elementary.edr_cast_as_string(elementary.edr_quote(metric_name)) }} as metric_name,
                        {{ elementary.edr_cast_as_string(elementary.edr_quote(metric_type)) }} as metric_type
                    from (
                        -- Column metrics
                        select
                            {%- if timestamp_column %}
                                filtered_data.edr_bucket_start as bucket_start,
                                filtered_data.edr_bucket_end as bucket_end,
                            {%- else %}
                                {{ elementary.null_timestamp() }} as bucket_start,
                                {{ elementary.edr_cast_as_timestamp(elementary.edr_quote(elementary.run_started_at_as_string())) }} as bucket_end,
                            {%- endif %}
                            {% if dimensions | length > 0 %}
                              {{ elementary.select_dimensions_columns(prefixed_dimensions, prefix="filtered_data") }},
                            {% endif %}
                            {%- if 'null_count' in metric_types -%} {{ elementary.null_count('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as null_count,
                            {%- if 'null_percent' in metric_types -%} {{ elementary.null_percent('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as null_percent,
                            {%- if 'not_null_percent' in metric_types -%} {{ elementary.not_null_percent('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as not_null_percent,
                            {%- if 'max' in metric_types -%} {{ elementary.max('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as max,
                            {%- if 'min' in metric_types -%} {{ elementary.min('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as min,
                            {%- if 'average' in metric_types -%} {{ elementary.average('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as average,
                            {%- if 'zero_count' in metric_types -%} {{ elementary.zero_count('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as zero_count,
                            {%- if 'zero_percent' in metric_types -%} {{ elementary.zero_percent('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as zero_percent,
                            {%- if 'not_zero_percent' in metric_types -%} {{ elementary.not_zero_percent('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as not_zero_percent,
                            {%- if 'standard_deviation' in metric_types -%} {{ elementary.standard_deviation('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as standard_deviation,
                            {%- if 'variance' in metric_types -%} {{ elementary.variance('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as variance,
                            {%- if 'max_length' in metric_types -%} {{ elementary.max_length('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as max_length,
                            {%- if 'min_length' in metric_types -%} {{ elementary.min_length('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as min_length,
                            {%- if 'average_length' in metric_types -%} {{ elementary.average_length('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as average_length,
                            {%- if 'missing_count' in metric_types -%} {{ elementary.missing_count('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as missing_count,
                            {%- if 'missing_percent' in metric_types -%} {{ elementary.missing_percent('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as missing_percent,
                            {%- if 'count_true' in metric_types -%} {{ elementary.count_true('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as count_true,
                            {%- if 'count_false' in metric_types -%} {{ elementary.count_false('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as count_false,
                            {%- if 'not_missing_percent' in metric_types -%} {{ elementary.not_missing_percent('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as not_missing_percent,
                            {%- if 'sum' in metric_types -%} {{ elementary.sum('filtered_data.' ~ column_obj.quoted) }} {%- else -%} null {% endif %} as sum
                        from (
                            -- Filtered data
                            {% if timestamp_column -%}
                                select 
                                    join_result.{{ column_obj.quoted }},
                                    {%- if dimensions -%} {{ elementary.select_dimensions_columns(dimensions, "dimension", prefix="join_result") }}, {%- endif -%}
                                    join_result.edr_bucket_start,
                                    join_result.edr_bucket_end
                                from (
                                    select 
                                        flt.{{ column_obj.quoted }},
                                        {%- if dimensions -%} {{ elementary.select_dimensions_columns(dimensions, "dimension", prefix="flt") }}, {%- endif -%}
                                        b.edr_bucket_start,
                                        b.edr_bucket_end
                                    from (
                                        select 
                                            mt.{{ column_obj.quoted }},
                                            {%- if dimensions -%} {{ elementary.select_dimensions_columns(dimensions, "dimension", prefix="mt") }}, {%- endif -%}
                                            {{ elementary.get_start_bucket_in_data(timestamp_column, min_bucket_start, metric_properties.time_bucket) }} as start_bucket_in_data
                                        from (
                                            select * 
                                            from {{ monitored_table }}
                                        ...
                                        where
                                            {{ elementary.edr_cast_as_timestamp(timestamp_column) }} >= (
                                                select min(min_bucket.edr_bucket_start) from (
                                                    select edr_bucket_start, edr_bucket_end
                                                    from ({{ elementary.complete_buckets_cte(metric_properties, min_bucket_start, max_bucket_end) }}) results
                                                    where edr_bucket_start >= {{ elementary.edr_cast_as_timestamp(min_bucket_start) }}
                                                      and edr_bucket_end <= {{ elementary.edr_cast_as_timestamp(max_bucket_end) }}
                                            ...
                                            and {{ elementary.edr_cast_as_timestamp(timestamp_column) }} < (
                                                select max(max_bucket.edr_bucket_end) from (
                                                    select edr_bucket_start, edr_bucket_end
                                                    from ({{ elementary.complete_buckets_cte(metric_properties, min_bucket_start, max_bucket_end) }}) results
                                                    where edr_bucket_start >= {{ elementary.edr_cast_as_timestamp(min_bucket_start) }}
                                                      and edr_bucket_end <= {{ elementary.edr_cast_as_timestamp(max_bucket_end) }}
                                    ...
                                    on (b.edr_bucket_start = flt.start_bucket_in_data)
                                ) join_result
                            {%- else %}
                                select 
                                    mt.{{ column_obj.quoted }},
                                    {%- if dimensions -%} {{ elementary.select_dimensions_columns(dimensions, "dimension", prefix="mt") }}, {%- endif -%}
                                    {{ elementary.null_timestamp() }} as edr_bucket_start,
                        ...
                        {% if dimensions | length > 0 %}
                            group by filtered_data.edr_bucket_start, filtered_data.edr_bucket_end
                            {%- for dimension in prefixed_dimensions %}
                    ...
                    where column_metrics.{{ metric_type }} is not null
                    {% if not loop.last %} union all {% endif %}
                {%- endfor %}
            {%- else %}
                {{ elementary.empty_table([('edr_column_name','string'),('bucket_start','timestamp'),('bucket_end','timestamp'),('bucket_duration_hours','int'),('dimension','string'),('dimension_value','string'),('metric_name','string'),('metric_type','string'),('metric_value','float')]) }}
            {%- endif %}
        ) column_metrics_unpivot
    ) metrics_final
{% endmacro %}