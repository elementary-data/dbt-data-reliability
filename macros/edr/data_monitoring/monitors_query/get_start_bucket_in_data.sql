{% macro get_start_bucket_in_data(timestamp_column, min_bucket_start, time_bucket) %}
    {% set bucket_start_datediff_expr %}
      floor({{ elementary.edr_timediff(time_bucket.period, min_bucket_start, elementary.edr_cast_as_timestamp(timestamp_column)) }} / {{ time_bucket.count }}) * {{ time_bucket.count }}
    {% endset %}
