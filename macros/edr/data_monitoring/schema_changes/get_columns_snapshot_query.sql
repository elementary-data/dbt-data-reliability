{% macro get_columns_snapshot_query(full_table_name) -%}
    {{ return(adapter.dispatch('get_columns_snapshot_query', 'elementary') (full_table_name)) }}
{%- endmacro %}

{% macro default__get_columns_snapshot_query(full_table_name) %}
...
{% macro sqlserver__get_columns_snapshot_query(full_table_name) %}
    ...
    {% set information_schema_columns %}
        (
            select * from {{ ref('filtered_information_schema_columns') }}
            where lower(full_table_name) = lower('{{ full_table_name }}')
