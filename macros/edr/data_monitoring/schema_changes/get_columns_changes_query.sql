...
{% macro get_column_changes_from_baseline_query(full_table_name, model_baseline_relation, include_added=False) -%}
    {{ return(adapter.dispatch('get_column_changes_from_baseline_query', 'elementary') (full_table_name, model_baseline_relation, include_added=False)) }}
{%- endmacro %}

{% macro default__get_column_changes_from_baseline_query(full_table_name, model_baseline_relation, include_added=False) %}
    {% set cur %}
        with baseline as (
...
{% macro sqlserver__get_columns_changes_query_generic(full_table_name, cur, pre, include_added=True) %}
    {%- set test_execution_id = elementary.get_test_execution_id() %}
    {%- set test_unique_id = elementary.get_test_unique_id() %}
    ...
    {% set columns_removed_filter_deleted_tables %}
    (
        {# This is filtering out the columns of tables that have been deleted. #}
        select
            removed.full_table_name,
    ...
    {% set column_changes_test_results %}
    (
        {# This is the query that is creating the test results table, by formatting a description and adding id + detection time #}
        select distinct
            {{ elementary.generate_surrogate_key(['full_table_name', 'column_name', 'change', 'detected_at']) }} as data_issue_id,
