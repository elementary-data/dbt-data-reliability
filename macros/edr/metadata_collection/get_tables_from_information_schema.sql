{% macro get_tables_from_information_schema(schema_tuple) %}
    {{
        return(
            adapter.dispatch("get_tables_from_information_schema", "elementary")(
                schema_tuple
            )
        )
    }}
...
{% macro default__get_tables_from_information_schema(schema_tuple) %}
    ...
    with
        ...
        information_schema_schemata as (
            ...
            select
                upper(catalog_name) as database_name, upper(schema_name) as schema_name
    ...
    full outer join
        information_schema_schemata as schemas
        on (
            tables.database_name = schemas.database_name
            and tables.schema_name = schemas.schema_name
        )
{% endmacro %}
...
{% macro databricks__get_tables_from_information_schema(schema_tuple) %}
    ...
    {% if is_catalog %}
        ...
        select
            ...
            upper(
                schemas.database_name || '.' || schemas.schema_name
            ) as full_schema_name,
            schemas.database_name as database_name,
            schemas.schema_name as schema_name,
            tables.table_name
    ...
    {% else %} {{ elementary.get_empty_tables_from_information_schema_table() }}
    {% endif %}
{% endmacro %}
