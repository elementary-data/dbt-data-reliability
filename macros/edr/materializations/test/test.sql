...
{% macro handle_anomaly_test(flattened_test) %}
    ...
    {% set metrics_table = elementary.get_elementary_test_table(
    ...
    {% for anomaly_scores_row in anomaly_scores_rows %}
        {% set anomaly_scores_group = (
            anomaly_scores_row.full_table_name,
            anomaly_scores_row.column_name,
            anomaly_scores_row.metric_name,
        ) %}
        {% do anomaly_scores_groups_rows.setdefault(anomaly_scores_group, []) %}
        {% do anomaly_scores_groups_rows[anomaly_scores_group].append(
            anomaly_scores_row
        ) %}
    {% endfor %}
    ...
    {% for anomaly_scores_group, anomaly_scores_rows in anomaly_scores_groups_rows.items() %}
        {% do elementary.debug_log(
            "Found {} anomaly scores for group {}.".format(
                anomaly_scores_rows | length, anomaly_scores_group
            )
        ) %}
...
{% macro handle_schema_changes_test(flattened_test) %}
    {% set schema_snapshots_tables_cache = elementary.get_cache("tables").get(
        "schema_snapshots"
    ) %}
    {% set schema_snapshots_table = elementary.get_elementary_test_table(
        elementary.get_elementary_test_table_name(), "schema_changes"
    ) %}
    ...
    {% for schema_changes_row in schema_changes_rows %}
        {% do elementary_test_results_rows.append(
            elementary.get_schema_changes_test_result_row(
...
{% macro handle_dbt_test(flattened_test) %}
    {% set result_rows = elementary.query_test_result_rows(
        sample_limit=elementary.get_config_var("test_sample_row_count")
    ) %}
...
{% macro materialize_test() %}
    {% if not elementary.is_elementary_enabled() %} {% do return(none) %} {% endif %}
...
{% materialization test, adapter = "snowflake" %}
    ...
    {%- else -%} {{ return(dbt.materialization_test_default()) }}
    {%- endif -%}
{% endmaterialization %}
...
{% macro get_anomaly_test_result_row(flattened_test, anomaly_scores_rows) %}
    ...
    {%- set full_table_name = elementary.insensitive_get_dict_value(
        latest_row, "full_table_name"
    ) %}
    ...
    {%- set column_name = elementary.insensitive_get_dict_value(
        latest_row, "column_name"
    ) %}
    {%- set metric_name = elementary.insensitive_get_dict_value(
        latest_row, "metric_name"
    ) %}
    {%- set test_unique_id = elementary.insensitive_get_dict_value(
        latest_row, "test_unique_id"
    ) %}
    {%- set has_anomaly_score = (
        elementary.insensitive_get_dict_value(latest_row, "anomaly_score")
        is not none
    ) %}
    {%- if not has_anomaly_score %}
        {% do elementary.edr_log(
            "Not enough data to calculate anomaly scores on `{}`".format(
                test_unique_id
            )
        ) %}
    {% endif %}
    ...
    {% set test_result_dict = {
        ...
        "other": elementary.insensitive_get_dict_value(
            latest_row, "anomalous_value"
        ),
...
{% macro get_schema_changes_test_result_row(
    flattened_test, schema_changes_row, schema_changes_rows
) %}
    {% set elementary_test_row = elementary.get_dbt_test_result_row(
        flattened_test, schema_changes_rows
    ) %}
