{% macro on_run_end() %}
  {%- if execute and flags.WHICH not in ['generate', 'serve'] %}
      ...
      {% set disable_elementary_on_run_end = elementary.get_config_var('disable_elementary_on_run_end') %}
      {% if not execute or edr_cli_run or disable_elementary_on_run_end %}
        {% do return("") %}
      {% endif %}
