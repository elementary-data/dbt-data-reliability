{% macro get_compiled_model_code_text(node) %}
    ...
    {% if not should_collect_model_sql %} {{ return(none) }} {% endif %}
    ...
    {% if not model_code %} {{ return(none) }} {% endif %}

    {% if model_sql_size_limit < model_code | length %}
        {{
            return(
                "Model code is too long - over " ~ model_sql_size_limit ~ " bytes"
            )
        }}
    {% else %} {{ return(model_code) }}
    {% endif %}
{% endmacro %}
