...
{% macro empty_data_monitoring_metrics(with_created_at=true) %}
    {{
        return(
            adapter.dispatch("empty_data_monitoring_metrics", "elementary")(
                with_created_at
            )
        )
    }}
...
{% macro empty_schema_columns_snapshot() %}
    {{
        elementary.empty_table(
            [
                ("column_state_id", "string"),
                ("full_column_name", "string"),
                ("full_table_name", "string"),
                ("column_name", "string"),
                ("data_type", "string"),
                ("is_new", "boolean"),
...
{% macro empty_column(column_name, data_type) %}
    ...
    {%- if data_type == "boolean" %}
        cast(
            {{ dummy_values["boolean"] }} as {{ elementary.edr_type_bool() }}
        ) as {{ column_name }}
    ...
    {%- elif data_type == "int" %}
        cast(
            {{ dummy_values["int"] }} as {{ elementary.edr_type_int() }}
        ) as {{ column_name }}
    {%- elif data_type == "bigint" %}
        cast(
            {{ dummy_values["bigint"] }} as {{ elementary.edr_type_bigint() }}
        ) as {{ column_name }}
    ...
    {%- elif data_type == "Nullable(float)" -%}
        cast(
            {{ dummy_values["float"] }} as Nullable({{ elementary.edr_type_float() }})
        ) as {{ column_name }}
    {%- elif data_type == "Nullable(int)" -%}
        cast(
            {{ dummy_values["int"] }} as Nullable({{ elementary.edr_type_int() }})
        ) as {{ column_name }}
    {%- else %}
        cast(
