...
{% macro redshift__complete_buckets_cte(time_bucket) %}
    {%- set complete_buckets_cte %}
      with integers as (
        ...
        limit {{ elementary.edr_timediff(time_bucket.period, elementary.edr_cast_as_timestamp(elementary.edr_quote(elementary.get_min_bucket_start())), elementary.edr_cast_as_timestamp(elementary.edr_quote(elementary.get_max_bucket_end()))) }} / {{ time_bucket.count }} + 1
      )
      select
