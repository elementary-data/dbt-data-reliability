...
{%- macro bigquery__get_default_config() -%}
    ...
    {% set default_config = elementary.dict_update(default_config, {'query_max_size': 250000}) %}
    {{- return(default_config) -}}
{%- endmacro -%}
...
{%- macro dremio__get_default_config() -%}
    {% set default_config = elementary.default__get_default_config() %}
    {% set default_config = elementary.dict_update(default_config, {'dbt_artifacts_chunk_size': 100}) %}
    ...
    {% set default_config = elementary.dict_update(default_config, {'cache_artifacts': false}) %}
