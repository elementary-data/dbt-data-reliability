...
{% macro default__daily_buckets_cte() -%}
    ...
    {%- set min_bucket_end = "'"~ (elementary.get_run_started_at() - modules.datetime.timedelta(elementary.get_config_var('days_back'))).strftime("%Y-%m-%dT00:00:00.000000Z") ~"'" %}
...
{% macro redshift__daily_buckets_cte() %}
    ...
    {%- set daily_buckets_cte %}
        {%- for i in range(0, days_back+1) %}
            {%- set daily_bucket = "'"~ (elementary.get_run_started_at() - modules.datetime.timedelta(i)).strftime("%Y-%m-%dT00:00:00.000000Z") ~"'" %}
