...
{% macro get_dbt_exposures_empty_table_query() %}
    {% set dbt_exposures_empty_table_query = elementary.empty_table(
        [
            ("unique_id", "string"),
            ("name", "string"),
            ("maturity", "string"),
...
{% macro flatten_exposure(node_dict) %}
    ...
    {% do flatten_exposure_metadata_dict.update(
        {
            "metadata_hash": elementary.get_artifact_metadata_hash(
                flatten_exposure_metadata_dict
            )
        }
    ) %}
