...
{% macro get_dbt_tests_empty_table_query() %}
    {% set dbt_tests_empty_table_query = elementary.empty_table(
        [
            ("unique_id", "string"),
...
{% macro flatten_test(node_dict) %}
    ...
    {% set test_metadata = elementary.safe_get_with_default(
    ...
    {% set default_description = elementary.get_default_description(
    ...
    {% set test_model_unique_ids = (
        elementary.get_parent_model_unique_ids_from_test_node(node_dict)
    ) %}
    {% set test_model_nodes = elementary.get_nodes_by_unique_ids(
        test_model_unique_ids
    ) %}
    ...
    {% set test_kwargs = elementary.safe_get_with_default(
        test_metadata, "kwargs", {}
    ) %}

    {% set primary_test_model_id = namespace(data=none) %}
    {% set override_primary_test_model_id = elementary.safe_get_with_default(
        config_dict, "override_primary_test_model_id"
    ) or elementary.safe_get_with_default(
        meta, "override_primary_test_model_id"
    ) %}
    ...
    {%- if primary_test_model_database is none or primary_test_model_schema is none -%}
        ...
        {% set primary_test_model_database, primary_test_model_schema = (
            elementary.get_model_database_and_schema_from_test_node(node_dict)
        ) %}
    {%- endif -%}
    ...
    {% set flatten_test_metadata_dict = {
        ...
        or test_kwargs.get("column_name"),
        ...
        "type": elementary.get_test_sub_type(
        ...
        or elementary.get_quality_dimension(
            test_original_name, test_namespace
        ),
        "group_name": group_name,
    } %}
    {% do flatten_test_metadata_dict.update(
        {
...
{% macro get_test_short_name(node_dict, test_metadata) %}
    ...
    {%- if generic_test_name %}
        {%- if test_package_name == "elementary" %} {{ return(generic_test_name) }}
        {%- elif test_package_name %}
            {% set test_short_name = (
                generic_test_name
                if (
                    test_instance_name.startswith(
                        test_package_name + "_" + generic_test_name
                    )
                    or test_instance_name.startswith(
                        test_package_name + "_source_" + generic_test_name
                    )
                )
        ...
        {%- else %}
            {% set test_short_name = (
                generic_test_name
                if (
                    test_instance_name.startswith(generic_test_name)
                    or test_instance_name.startswith(
                        "source_" + generic_test_name
                    )
                )
                else test_instance_name
            ) %}
    ...
    {%- else %} {{ return(test_instance_name) }}
    {%- endif %}
{% endmacro %}
...
{% macro get_default_description(test_original_name, test_namespace=none) %}
    {% set description = none %}
    {% set common_test_config = (
        elementary.get_common_test_config_by_namespace_and_name(
            test_namespace, test_original_name
        )
    ) %}
