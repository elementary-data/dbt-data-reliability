                ...
                {% if elementary.get_elementary_relation(artifacts_model) %}
                    {% if artifacts_hashes is not none %}
                        {% do upload_artifacts_func(
                            should_commit=true,
                            metadata_hashes=artifacts_hashes.get(
                                artifacts_model, []
                            ),
                        ) %}
                    {% else %} {% do upload_artifacts_func(should_commit=true) %}
                    {% endif %}
                {% endif %}
            {% else %}
                {% do elementary.file_log(
                    "[{}] Artifacts already ran.".format(artifacts_model)
                ) %}
            {% endif %}
        {% endfor %}
        {% if elementary.get_dbt_columns_materialized() != "view" and elementary.get_config_var(
            "upload_dbt_columns"
        ) %}
...
{% macro get_artifacts_hashes() %}
    ...
    {% if not local_md5 %} {% do return(none) %} {% endif %}

    {% set database_name, schema_name = elementary.get_package_database_and_schema() %}
    {% set artifacts_hash_relation = adapter.get_relation(
        database_name, schema_name, "dbt_artifacts_hashes"
    ) %}
    {% if not artifacts_hash_relation %} {% do return(none) %} {% endif %}
    ...
    {% for artifacts_model, metadata_hashes in artifact_agate_hashes.items() %}
        {% do artifacts_hashes.update(
            {artifacts_model: metadata_hashes.columns["metadata_hash"]}
        ) %}
    {% endfor %}
