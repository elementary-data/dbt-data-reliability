...
{% macro flatten_table_columns(table_node) %}
    ...
    {% if not column_nodes %} {% do return(none) %} {% endif %}
    ...
    {% for column_node in column_nodes.values() %}
        ...
        {% if elementary.get_config_var(
            "columns_upload_strategy"
        ) == "all" or column_node.get("description") or has_meta or has_tags %}
...
{% macro flatten_column(table_node, column_node) %}
    ...
    {% set flatten_column_metadata_dict = {
        ...
        "unique_id": "column.{}.{}".format(
            table_node.get("unique_id"), column_node.get("name")
        ),
    ...
    {% do flatten_column_metadata_dict.update(
        {
            "metadata_hash": elementary.get_artifact_metadata_hash(
                flatten_column_metadata_dict
            )
        }
    ) %}
