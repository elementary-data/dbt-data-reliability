...
{% macro freshness_description() %}
    ...
    || anomalous_value
    || ', '
    || abs(round({{ elementary.edr_cast_as_numeric("metric_value/3600") }}, 2))
    || ' hours ago. Usually the table is updated within '
    || abs(round({{ elementary.edr_cast_as_numeric("training_avg/3600") }}, 2))
    || ' hours.'
{% endmacro %}
