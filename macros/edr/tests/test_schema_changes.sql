{% test schema_changes(model) %}
    ...
    {%- if execute and flags.WHICH in ["test", "build"] %}
        ...
        {%- set column_snapshot_query = elementary.get_columns_snapshot_query(
            full_table_name
        ) %}
        ...
        {% set schema_changes_alert_query = (
            elementary.get_columns_changes_from_last_run_query(
                full_table_name, temp_table_relation
            )
        ) %}
        {{
        ...
        select *
        from {{ alerts_temp_table_relation }}
