...
{% macro get_anomalies_test_configuration(
    ...
    {%- set where_expression = elementary.get_test_argument(
        "where_expression", where_expression, model_graph_node
    ) %}
    ...
    {%- set min_training_set_size = elementary.get_test_argument(
        "min_training_set_size", min_training_set_size, model_graph_node
    ) %}
    {%- set backfill_days = elementary.get_test_argument(
        "backfill_days", backfill_days, model_graph_node
    ) %}
    ...
    {%- set seasonality = elementary.get_seasonality(
        seasonality, model_graph_node, time_bucket, timestamp_column
    ) %}
    ...
    {%- set test_configuration = elementary.empty_dict_keys_to_none(
        test_configuration
    ) -%}
    ...
    {%- set metric_properties = elementary.empty_dict_keys_to_none(
        metric_properties
    ) -%}
