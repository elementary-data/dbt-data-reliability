{% test dimension_anomalies(
    ...
    {%- if execute and elementary.is_test_command() and elementary.is_elementary_enabled() %}
        ...
        {%- set dimension_monitoring_query = elementary.dimension_monitoring_query(
            model,
            model_relation,
            metric_properties.dimensions,
            min_bucket_start,
            max_bucket_end,
            metric_properties,
        ) %}
        ...
        {{
            elementary.debug_log(
                "dimension monitors anomaly scores query - \n" ~ anomaly_scores_query
            )
        }}
        ...
        {% set anomalous_dimension_rows_sql = (
            elementary.get_anomaly_query_for_dimension_anomalies(
                flattened_test
            )
        ) %}
        ...
        {% do elementary.store_anomaly_test_results(
            flattened_test, anomalous_dimension_rows_sql
        ) %}
