        ...
        {%- for exposure in matching_exposures -%}
            ...
            {%- if meta != none and (meta['referenced_columns'] or none) is iterable -%}
                {%- for exposure_column in meta['referenced_columns'] -%}
                    ...
                    #}
                    {%- if exposure['depends_on']['nodes'] | length > 1 and 'node' not in exposure_column -%}
                    ...
                    {%- elif exposure['depends_on']['nodes'] | length == 1 or (context['render'](elementary.get_rendered_ref((exposure_column['node'] or ''))) == context['render'](model | lower)) -%}
