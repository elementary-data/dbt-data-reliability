{% test json_schema(model, column_name, where_expression) %}
    ...
    {% if model is string %}
        {{
            exceptions.raise_compiler_error(
                "Unsupported model: "
                ~ model
                ~ " (this might happen if you provide a 'where' parameter to the test or override 'ref' or 'source')"
            )
        }}
    {% endif %}
    ...
    {% if not elementary.column_exists_in_relation(model, column_name) %}
        {% do exceptions.raise_compiler_error(
            "Column '{}' does not exist in node '{}'!".format(
    ...
    {% if not kwargs %}
        {% do exceptions.raise_compiler_error(
            "A json schema must be supplied as a part of the test!"
        ) %}
    {% endif %}

    {{
        elementary.test_python(
            model,
            elementary.json_schema_python_test,
            {"column_name": column_name, "json_schema": kwargs},
            where_expression,
            packages=["jsonschema"],
        )
    }}
...
{% macro json_schema_python_test(args) %}
    ...
    raw_json_schema
    = r"""{{ args.json_schema if args.json_schema is string else tojson(args.json_schema) }}"""
    ...
    model_df["is_valid_json"]
    = model_df[column_name].apply(lambda val:is_valid_json(val, json_schema))
