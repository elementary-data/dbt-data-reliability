{% test table_anomalies(model, table_anomalies, freshness_column, timestamp_column, sensitivity, backfill_days, where_expression, time_bucket) %}
    ...
    {%- if execute and flags.WHICH in ['test', 'build'] %}
        ...
        {%- set min_bucket_start = "'" ~ elementary.get_test_min_bucket_start(model_graph_node, backfill_days, table_monitors) ~ "'" %}
        ...
        {% set anomaly_scores_query = elementary.get_anomaly_scores_query(temp_table_relation, model_graph_node, sensitivity, backfill_days, table_monitors, time_bucket=time_bucket) %}
