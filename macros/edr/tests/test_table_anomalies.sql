{% test table_anomalies(
    ...
    {%- if execute and elementary.is_test_command() and elementary.is_elementary_enabled() %}
        ...
        {% if not model_relation %}
            {{
                exceptions.raise_compiler_error(
                    "The test has unsupported configuration, please contact Elementary support"
                )
            }}
        {% endif %}
        ...
        {% for table_monitor in table_monitors %}
            {% do table_metrics.append(
                {"type": table_monitor, "name": table_monitor}
            ) %}
        {% endfor %}
        ...
        {{
            elementary.debug_log(
                "table monitors anomaly scores query - \n" ~ anomaly_scores_query
            )
        }}
