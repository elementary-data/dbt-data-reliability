...
{% macro get_data_freshness_sla_query(model_relation, timestamp_column, sla_deadline_utc, target_date, target_date_start_utc, target_date_end_utc, deadline_passed, formatted_sla_time, timezone, where_expression) %}
    ...
    max_data_timestamp as (
        select
            max({{ elementary.edr_cast_as_timestamp(timestamp_column) }}) as max_timestamp
        from {{ model_relation }}
        {% if where_expression %}
    ...
    freshness_result as (
        select
            ...
            mdt.max_timestamp,
            case
                when mdt.max_timestamp is null then 'NO_DATA'
                {# Data from today or future (e.g. pre-loaded records) counts as fresh #}
                when mdt.max_timestamp >= sd.target_date_start_utc then 'DATA_FRESH'
                else 'DATA_STALE'
    ...
    final_result as (
        select
            ...
            freshness_status,
            cast(max_timestamp as {{ elementary.edr_type_string() }}) as max_timestamp,
            case
