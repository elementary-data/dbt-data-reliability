{% macro handle_tests_results() %}
    ...
    {{
        elementary.insert_data_monitoring_metrics(
            database_name, schema_name, test_metrics_tables
        )
    }}
    {{
        elementary.insert_schema_columns_snapshot(
            database_name, schema_name, test_columns_snapshot_tables
        )
    }}
    {% if test_result_rows %}
        {% set test_result_rows_relation = adapter.get_relation(
            database=database_name,
            schema=schema_name,
            identifier="test_result_rows",
        ) %}
        {% do elementary.insert_rows(
            test_result_rows_relation, test_result_rows, should_commit=True
        ) %}
    {% endif %}
    {% if elementary_test_results %}
        {% set elementary_test_results_relation = adapter.get_relation(
            database=database_name,
            schema=schema_name,
...
{% macro get_result_enriched_elementary_test_results(
    ...
    {% for result in results | selectattr("node.resource_type", "==", "test") %}
        ...
        {% set elementary_test_failed_row_count = (
            cached_elementary_test_failed_row_counts.get(
                result.node.unique_id
            )
        ) %}
...
{% macro insert_data_monitoring_metrics(
    ...
    {% if not target_relation %}
        {% do exceptions.raise_compiler_error(
            "Couldn't find Elementary's models. Please run `dbt run -s elementary`."
        ) %}
    {% endif %}
    ...
    {%- do elementary.run_query(
        dbt.create_table_as(True, temp_relation, test_tables_union_query)
    ) %}
...
{% macro insert_schema_columns_snapshot(
    ...
    {%- set target_relation = adapter.get_relation(
        database=database_name,
        schema=schema_name,
        identifier="schema_columns_snapshot",
    ) -%}
    {% if not target_relation %}
        {% do exceptions.raise_compiler_error(
            "Couldn't find Elementary's models. Please run `dbt run -s elementary`."
