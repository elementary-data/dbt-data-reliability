{% macro handle_tests_results() %}
    ...
    {% endif %}
    {% if test_result_rows %}
      {% set test_result_rows_relation = elementary.get_elementary_relation(database=database_name, schema=schema_name, identifier='test_result_rows') %}
      {% do elementary.insert_rows(test_result_rows_relation, test_result_rows, should_commit=True) %}
    {% endif %}
    {% if elementary_test_results %}
      {% set elementary_test_results_relation = elementary.get_elementary_relation(database=database_name, schema=schema_name, identifier='elementary_test_results') %}
...
{% macro insert_data_monitoring_metrics(database_name, schema_name, test_metrics_tables) %}
    ...
    {%- set target_relation = elementary.get_elementary_relation(database=database_name, schema=schema_name, identifier='data_monitoring_metrics') -%}
...
{% macro insert_schema_columns_snapshot(database_name, schema_name, test_columns_snapshot_tables) %}
    ...
    {%- set target_relation = elementary.get_elementary_relation(database=database_name, schema=schema_name, identifier='schema_columns_snapshot') -%}
    {% if not target_relation %}
      {% do exceptions.raise_compiler_error("Couldn't find Elementary's models in `" ~ elementary.target_database() ~ "." ~ target.schema ~ "`. Please run `dbt run -s elementary --target " ~ target.name ~ "`.") %}
    {% endif %}
