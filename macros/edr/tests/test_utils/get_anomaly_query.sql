...
{% macro get_read_anomaly_scores_query(flattened_test=none) %}
    ...
    {% set anomaly_direction = elementary.get_test_argument(argument_name='anomaly_direction', value=flattened_test.test_params.anomaly_direction) %}
    ...
    {%- set anomaly_query -%}
      ...
      select
        ...
        -- when there's an anomaly we would want to use the last value of the metric (lag), otherwise visually the expactations would look out of bounds
        case
        ...
        case when is_anomalous = TRUE and '{{ anomaly_direction }}' != 'drop' then
