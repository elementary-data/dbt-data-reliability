{% macro backfill_result_rows() %}
    ...
    {% if is_incremental() %} {% do return("") %} {% endif %}

    {% set elementary_test_results_relation = ref("elementary_test_results") %}
    {% set result_row_exists = (
        elementary.get_column_in_relation(
            elementary_test_results_relation, "result_rows"
        )
        is not none
    ) %}
    {% if not result_row_exists %} {% do return("") %} {% endif %}
            ...
            {% do test_result_rows_rows.append(
                {
                    ...
                    "detected_at": elementary_test_results_row.detected_at.strftime(
                        elementary.get_time_format()
                    ),
