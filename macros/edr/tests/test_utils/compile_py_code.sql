{% macro snowflake__compile_py_code(
    model, py_code, output_table, where_expression, code_type
) %}
    import pandas
    ...
    output_df.write.mode(
        'overwrite').save_as_table(target_relation, table_type = 'temporary')

    def get_fail_count(test_output):
    if isinstance(test_output, int):
    return test_output
    if isinstance(test_output, bool):
    return 0 if test_output
    else
        1
        ...
        raise valueerror(
            'Received invalid return value, expected either DataFrame or a boolean.'
        )
...
{% macro bigquery__compile_py_code(
    model, py_code, output_table, where_expression, code_type
) %}
    import pandas
    import pyspark.sql
    ...
    output_df.write.mode(
        ...
        'writeMethod',
        'direct'
    ).option('writeDisposition', 'WRITE_TRUNCATE').save(target_relation)
...
{% macro default__compile_py_code(
    model, py_code, output_table, where_expression, code_type
) %}
    {{
        exceptions.raise_compiler_error(
            "Elementary's Python tests are not yet supported on %s." % target.type
        )
    }}
{% endmacro %}
