    ...
    {% if execute and elementary.is_test_command() and elementary.is_elementary_enabled() %}
        ...
        {%- if not event_timestamp_column -%}
            {%- do exceptions.raise_compiler_error(
                "event_timestamp_column must be specified for the event freshness test!"
            ) -%}
        {%- endif -%}
        {%- set event_timestamp_column_data_type = (
            elementary.find_normalized_data_type_for_column(
                model_relation, event_timestamp_column
            )
        ) -%}
        {%- if not elementary.is_column_timestamp(
            model_relation,
            event_timestamp_column,
            event_timestamp_column_data_type,
        ) -%}
            {% do exceptions.raise_compiler_error(
                "Column `{}` is not a timestamp.".format(
                    event_timestamp_column
                )
            ) %}
        {%- endif -%}
