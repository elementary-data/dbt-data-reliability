{% test all_columns_anomalies(
    ...
    {%- if execute and elementary.is_test_command() and elementary.is_elementary_enabled() %}
        ...
        {%- set column_objs_and_monitors = (
            elementary.get_all_column_obj_and_monitors(
                model_relation, column_anomalies
            )
        ) -%}
        ...
        {%- if column_objs_and_monitors | length > 0 %}
            ...
            {%- for column_obj_and_monitors in column_objs_and_monitors %}
                ...
                {%- set ignore_column = elementary.should_ignore_column(
                    column_name, exclude_regexp, exclude_prefix
                ) -%}
                {%- if not ignore_column -%}
                    {%- do monitors.extend(column_monitors) -%}
                    {% if test_configuration.timestamp_column %}
                        {%- set min_bucket_start, max_bucket_end = (
        ...
        {%- set anomaly_scores_query = elementary.get_anomaly_scores_query(
...
{%- macro should_ignore_column(column_name, exclude_regexp, exclude_prefix) -%}
    ...
    {%- if exclude_regexp -%}
        {%- set is_match = regex_module.match(
            exclude_regexp, column_name, regex_module.IGNORECASE
        ) %}
        {%- if is_match -%} {{ return(True) }} {%- endif -%}
    {%- endif -%}
    {% if exclude_prefix %}
