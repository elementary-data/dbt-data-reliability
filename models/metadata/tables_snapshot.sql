...
select
    ...
    {% if is_incremental() %}
        ...
        {%- set known_schemas_query %}
            ...
            where detected_at = (select max(detected_at) from {{ this }})
        {% endset %}
        {%- set known_schemas = result_column_to_list(known_schemas_query) %}

        case when
            full_table_name not in {{ strings_list_to_tuple(known_tables) }}
            and full_schema_name in {{ strings_list_to_tuple(known_schemas) }}
        then true
        else false end
        as is_new
    {% else %}
