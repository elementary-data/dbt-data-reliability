...
from .utils import create_test_table, insert_rows, update_var, lowercase_column_names, assert_dfs_equal, \
    agate_table_to_pandas_dataframe, get_package_database_and_schema

MIN_BUCKET_START = datetime(2022, 1, 1, 0, 0, 0)
RUN_STARTED_AT = datetime(2022, 1, 4, 0, 13, 42)
DATA_MONITORING_METRICS_COLUMNS_TO_TYPES = [('id', 'string'),  # same as empty_table.sql::empty_data_monitoring_metrics
...
BASE_DIR = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'anomaly_scores_query_test_cases')
...
def test_anomaly_scores_query(dbt_project: DbtProject,
    ...
    my_test_metrics_relation = create_test_table(dbt_project,
                                 "my_test_metrics_table_relation",
                                 {x: y for x, y in DATA_MONITORING_METRICS_COLUMNS_TO_TYPES})
    my_test_data_monitoring_metrics = create_test_table(dbt_project,
