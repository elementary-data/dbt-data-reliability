{% test config_levels(
    ...
    {%- if execute and flags.WHICH in ["test", "build"] %}
        ...
        {%- set configs_to_test = [
            ("timestamp_column", metric_properties_dict.timestamp_column),
            ...
            (
                "min_training_set_size",
                configuration_dict.min_training_set_size,
            ),
        ...
        {%- for config in configs_to_test %}
            ...
            {%- set config_check = compare_configs(
                config_name, config_value, expected_config
            ) %}
            {%- if config_check %}
        ...
        {%- if unexpected_config | length > 0 %}
            {%- do exceptions.raise_compiler_error(
                "Failure config_levels: " ~ unexpected_config
            ) -%}
        {%- else %}
...
{% macro compare_configs(config_name, config, expected_config) %}
    {%- if config != expected_config.get(config_name) %}
        {%- set unexpected_message = (
            "For {0} - got config: {1}, expected config: {2}"
        ).format(config_name, config, expected_config.get(config_name)) %}
