{% macro assert_value_in_list(value, list, context="") %}
    ...
    {% if upper_value in list or lower_value in list %}
        {% do elementary.edr_log(
            context ~ " SUCCESS: " ~ upper_value ~ " in list " ~ list
        ) %}
        {{ return(0) }}
    {% else %}
        {% do elementary.edr_log(
            context ~ " FAILED: " ~ upper_value ~ " not in list " ~ list
        ) %}
        {{ return(1) }}
    {% endif %}
{% endmacro %}
...
{% macro assert_lists_contain_same_items(list1, list2, context="") %}
    ...
    {% endif %}
    {% for item1 in list1 %}
        {% if item1 is string %} {% set item1 = item1 | lower %} {% endif %}
        {% if item1 not in list2 %}
            {% do elementary.edr_log(
                context ~ " FAILED: " ~ item1 ~ " not in list " ~ list2
            ) %}
            {{ return(1) }}
        {% endif %}
    {% endfor %}
...
{% macro assert_list1_in_list2(list1, list2, context="") %}
    ...
    {% if not list1 or not list2 %}
        {% do elementary.edr_log(
            context ~ " FAILED: list1 is empty or list2 is empty"
        ) %}
        {{ return(1) }}
    {% endif %}
    {% for item1 in list1 %}
