{% macro validate_schema_changes() %}
    {% set expected_changes = {
        ("schema_changes", "red_cards"): "column_added",
    ...
    {% if error_alert_rows | length != 1 %}
        {% do elementary.edr_log(
            "FAILED: for schema_changes_from_baseline with enforce_types true - no error eccured"
        ) %}
        {{ return(1) }}
    {% endif %}
    ...
    {% for row in failure_alert_rows %}
        ...
        {% if (test_short_name, column_name) not in expected_changes %}
            {% do elementary.edr_log(
                "FAILED: "
                ~ test_short_name
                ~ " - could not find expected alert for "
                ~ column_name
                ~ ", "
                ~ alert
            ) %}
        ...
        {% if expected_changes[(test_short_name, column_name)] != alert %}
            {% do elementary.edr_log(
                ...
                ~ column_name
                ~ " expected alert type "
                ~ expected_changes[(test_short_name, column_name)]
                ~ " but got "
                ~ alert
            ) %}
            {{ return(1) }}
    ...
    {% if found_schema_changes %}
        ...
        {%- if missing_changes | length == 0 %}
            {% do elementary.edr_log(
                "SUCCESS: all expected schema changes were found - "
                ~ found_schema_changes
            ) %}
            {{ return(0) }}
        {%- endif %}
    {% endif %}
