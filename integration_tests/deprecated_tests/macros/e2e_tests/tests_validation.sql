{% macro validate_no_timestamp_anomalies() %}
    ...
    {% endset %}
    {% set results = elementary.result_column_to_list(
        no_timestamp_row_count_validation_query
    ) %}
    ...
    {% for row in alert_rows %}
        ...
        {% else %} {% do indexed_columns.update({column_name: [alert]}) %}
        {% endif %}
    {% endfor %}
    {% set results = [] %}
...
{% macro validate_error_test() %}
    ...
    {% endset %}
    {% set results = (
        elementary.result_column_to_list(error_test_validation_query)
        | unique
        | list
    ) %}
...
{% macro validate_error_snapshot() %}
    ...
    {% endset %}
    {% set results = elementary.result_column_to_list(
        error_snapshot_validation_query
    ) %}
    {{ assert_lists_contain_same_items(results, ["error"]) }}
{% endmacro %}
...
{% macro validate_regular_tests() %}
    ...
    {% set found_tests = [] %}
    {% for row in alert_rows %}
        {%- if row[0] -%} {% do found_tables.append(row[0]) %} {%- endif -%}
        {%- if row[1] -%} {% do found_columns.append(row[1]) %} {%- endif -%}
        {%- if row[2] -%} {% do found_tests.append(row[2]) %} {%- endif -%}
    {% endfor %}
    {{
        assert_list1_in_list2(
            [
                "string_column_anomalies",
                "numeric_column_anomalies",
                "any_type_column_anomalies",
                "any_type_column_anomalies_validation",
                "numeric_column_anomalies_training",
            ],
            found_tables,
        )
    }}
    ...
    {{
        assert_list1_in_list2(
            [
                "relationships",
                "singular_test_with_no_ref",
                "singular_test_with_one_ref",
                "singular_test_with_two_refs",
                "singular_test_with_source_ref",
...
{% macro validate_dbt_artifacts() %}
    ...
    {{
        assert_value_in_list(
            "any_type_column_anomalies", models, context="dbt_models"
        )
    }}
    ...
    {{
        assert_value_in_list(
            "any_type_column_anomalies_training", sources, context="dbt_sources"
        )
    }}
    {{
        assert_value_in_list(
            "string_column_anomalies_training", sources, context="dbt_sources"
        )
    }}
    {{
        assert_value_in_list(
            "any_type_column_anomalies_validation", sources, context="dbt_sources"
        )
    }}
    ...
    {{
        assert_list1_in_list2(
            run_results, all_executable_nodes, context="dbt_run_results"
        )
    }}
