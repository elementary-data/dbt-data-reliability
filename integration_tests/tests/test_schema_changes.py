...
def test_schema_changes(test_id: str, dbt_project: DbtProject):
    ...
    test_result = dbt_project.test(
        test_id, dbt_test_name, data=DATASET1, create_real_model=True
    )
    ...
    test_results = dbt_project.test(
        ...
        dbt_test_name,
        data=DATASET2,
        create_real_model=True,
        multiple_results=True,
    )
    assert_test_results(test_results)
    test_result = dbt_project.test(test_id, dbt_test_name, create_real_model=True)
    assert test_result["status"] == "pass"
